...
export async function removeConnection(redis: Redis, userId: string, connId: string): Promise<void> {
  await redis.hdel(`presence:conns:${userId}`, connId);
}

export async function updateActivity(redis: Redis, userId: string, connId: string): Promise<void> {
  await redis.hset(`presence:conns:${userId}`, connId, Date.now());
}
